...
def random_pairs_of_minibatches(minibatches): # from domainbed.lib.misc import random_pairs_of_minibatches
...
class ERM(Algorithm):
    ...
    def calculate_gradient_for_influence(self, minibatches):
        '''
        For each group, calculate their averaged gradient.
        '''
        ##################################
        # TracIn 
        ...
        for i in range(batch_size): # for each data point 
            x = all_x[i:i+1]
            y = all_y[i:i+1]
            with torch.no_grad():
            ...
            self.optimizer.zero_grad()
            loss.backward()
            # grad = features.grad.view(-1).cpu().numpy()
            grad = self.classifier.weight.grad.view(-1).cpu().numpy()
            self.optimizer.zero_grad() # clean up
